...
async function main() {
  const limitArg = (process.argv[2] || "all").toLowerCase();
  const outPath = process.argv[3] || `exports/interactive_questions_${limitArg}.csv`;
  ...
  console.log(`✓ Wrote ${rows.length} interactive questions -> ${abs}`);
  await pool.end();
}
